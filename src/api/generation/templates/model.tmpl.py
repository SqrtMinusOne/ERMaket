...
class {{ Names.class_name(schema, table.name) }}(Base):
    ...
    {{ Names.attribute_name(col.name) }} = sa.Column(sa.{{ sa_type(col.type_) }}, sa.ForeignKey('{{ schema }}.{{ col.fk.table.name }}.{{ col.fk.column.name }}', ondelete='{{ col.fk.ondelete }}', onupdate='{{ col.fk.onupdate }}'), primary_key={{ col.pk }}, unique={{ col.unique }})
    {%- endfor %}
    {% for col in table.foreign_keys %}
